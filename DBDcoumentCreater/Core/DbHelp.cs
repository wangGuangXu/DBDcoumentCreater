...
namespace Core.DB
{
    public partial class DbHelp : IDisposable
    {
        enum DBType
        {
            SqlServer2000,
        ...
        #region 成员定义
        private DbProviderFactory _provider;//数据库Provider
        private DBType _dbType;//数据库类型
        private char _paramPrefix = '@';//参数符号
        ...
        public DbHelp(string connectionName = "local")
        {
            //默认使用ConnectionString第一项
            var config = ConfigurationManager.ConnectionStrings[connectionName];
            if (config == null)
            {
                throw new Exception("can't find " + connectionName + " in connectionStrings section ");
            }
            else if (string.IsNullOrEmpty(config.ProviderName))
            {
                throw new Exception("providerName is empty in connectionStrings section ");
            }

            _provider = DbProviderFactories.GetFactory(config.ProviderName);
        ...
        private void CommonConstruct(string _dbtype = "")
        {
            ...
            if (_dbtype.StartsWith("Oracle")) _dbType = DBType.Oracle;
            else if (_dbtype.StartsWith("SQLite")) _dbType = DBType.SQLite;
            else if (_dbtype.StartsWith("System.Data.SqlClient")) _dbType = DBType.SqlServer;
            // else try with provider name
            else if (_dbtype.IndexOf("Oracle", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.Oracle;
            else if (_dbtype.IndexOf("SQLite", StringComparison.InvariantCultureIgnoreCase) >= 0) _dbType = DBType.SQLite;
        ...
        public object this[string name, DbType dbType, ParameterDirection direction]
        {
            set
            {
                if (name[0] != _paramPrefix) name = _paramPrefix + name;

                var para = _provider.CreateParameter();
        ...
        private DbParameter CreateParameter(string name, object value, DbType t = DbType.Object, ParameterDirection pDirection = ParameterDirection.Input)
        {
            ...
            if (name[0] == _paramPrefix)
            {
                para.ParameterName = name;
            }
            else
            {
                para.ParameterName = _paramPrefix + name;
            }
            para.Value = value;
            return para;
        }
        ...
        private DataTable Execute(DbCommand cmd)
        {
            try
            {
                using (var adapter = _provider.CreateDataAdapter())//创建适配器
                {
        ...
        private string getSqlOnly(DbCommand cmd)
        {
            ...
            foreach (DbParameter para in cmd.Parameters)
            {
                ...
                else if (_paramPrefix == '@' || para.DbType == DbType.AnsiString || para.DbType == DbType.String || para.DbType == DbType.StringFixedLength)
                {
                    sql = sql.Replace(para.ParameterName, "'" + para.Value.ToString() + "'");
                }
                else if (_paramPrefix == ':' || para.DbType == DbType.DateTime || para.DbType == DbType.DateTime2 || para.DbType == DbType.DateTimeOffset)
                {
                    //排除未知的时间类型
                    DateTime time;
        ...
        {
            var sqls = CreatePageSql(sql, start, limit);
            using (var cmd = CreateCommand(sqls[1]))
            {
                CreatePageParameter(null, start, limit, cmd);
                return Execute(cmd);
            }
        }
        ...
        public DataTable ExecuteSql(string sql, int start, int limit, out int count)
        {
            ...
            var sqls = CreatePageSql(sql, start, limit, true);
        ...
        private string[] SplitSqlForPaging(string sql)
        {
            ...
            if (rxDistinct.IsMatch(sqlInfo[2]))
                sqlInfo[0] = sql.Substring(0, g.Index) + "COUNT(" + m.Groups[1].ToString().Trim() + ")  TOTAL " + sql.Substring(g.Index + g.Length);
            else
                sqlInfo[0] = sql.Substring(0, g.Index) + "COUNT(*) TOTAL " + sql.Substring(g.Index + g.Length);
        ...
        private bool CreatePageSqlReverse(string sql, ref string[] sqls, int start, int limit, int total = 0)
        {
            //如果总行数不多或分页的条数位于前半部分，没必要逆序分页
            if (total < 200000 || start <= total / 2)
            {
                return false;
            }
            ...
            if (_dbType == DBType.SqlServer2000)
            {
        ...
        private string[] CreatePageSql(string sql, int start, int limit, bool createCount = false)
        {
            //需要输出的sql数组
            string[] sqls = null;

            //生成count的SQL语句 SqlServer生成分页，必须通过正则拆分
            if (createCount || _dbType == DBType.SqlServer || _dbType == DBType.SqlServer2000)
            {
                sqls = SplitSqlForPaging(sql);
                if (sqls == null)
                {
        ...
        private void CreatePageParameter(IDictionary<string, object> condition, int start, int limit, DbCommand cmd = null)
        {
            ...
            if (_dbType == DBType.SQLite)
            {
                if (condition != null)
                {
                    condition["PageLimit"] = limit;
                    condition["PageStart"] = start;
                }
                else if (cmd != null)
                {
                    var para = cmd.CreateParameter();
                    para.ParameterName = "PageLimit";
                    para.Value = limit;
                    cmd.Parameters.Add(para);

                    para = cmd.CreateParameter();
                    para.ParameterName = "PageStart";
                    para.Value = start;
                    cmd.Parameters.Add(para);
                }
            }
            else if (_dbType == DBType.SqlServer2000)
            {
                //paraName = _paramPrefix + "PageLimit";
                paraName = "PageLimit";
                paraValue = start + 1;

                if (condition != null) condition[paraName] = paraValue;
                else if (cmd != null)
                {
                    var para = cmd.CreateParameter();
            ...
            {
                //paraName = _paramPrefix + "PageStart";
                paraName = "PageStart";
                paraValue = start + 1;
            ...
            {
                //paraName = _paramPrefix + "PageEnd";
                paraName = "PageEnd";
                paraValue = start + limit;

                if (condition != null) condition[paraName] = start + limit;
                if (cmd != null)
                {
                    var para = cmd.CreateParameter();
                    para.ParameterName = paraName;
